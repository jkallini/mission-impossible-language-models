...
if __name__ == "__main__":
    ...
    parser.add_argument('perturbation_type',
                        ...
                        const='all',
                        nargs='?',
                        choices=["reverse_control", "reverse_full", "reverse_partial"],
    ...
    test_files = sorted(glob(
        f"{BABYLM_DATA_PATH}/babylm_data_perturbed/babylm_reverse_control/babylm_test_affected/*.test"))
    ...
    for test_file in test_files:
        ...
        file_token_sequences = [
            [int(s) for s in l.split() if int(s) != marker_rev_token]
            for l in f.readlines()]
        
        # Reverse Control
        if args.perturbation_type == "reverse_control" and args.test_perturbation_type == "reverse_control":
            file_token_sequences = [seq + [marker_rev_token] for seq in file_token_sequences]
        elif args.perturbation_type == "reverse_control" and args.test_perturbation_type == "reverse_full":
            file_token_sequences = [[marker_rev_token] + seq for seq in file_token_sequences]
        elif args.perturbation_type == "reverse_control" and args.test_perturbation_type == "reverse_partial":
            file_token_sequences = [seq[:len(seq)//2] + [marker_rev_token] + seq[len(seq)//2:] for seq in file_token_sequences]

        # Reverse Full
        elif args.perturbation_type == "reverse_full" and args.test_perturbation_type == "reverse_control":
            file_token_sequences = [seq[::-1] + [marker_rev_token] for seq in file_token_sequences]
        elif args.perturbation_type == "reverse_full" and args.test_perturbation_type == "reverse_full":
            file_token_sequences = [[marker_rev_token] + seq[::-1] for seq in file_token_sequences]
        elif args.perturbation_type == "reverse_full" and args.test_perturbation_type == "reverse_partial":
            file_token_sequences = [seq[::-1] for seq in file_token_sequences]
            file_token_sequences = [seq[:len(seq)//2] + [marker_rev_token] + seq[len(seq)//2:] for seq in file_token_sequences]

        # Reverse Partial
        elif args.perturbation_type == "reverse_partial" and args.test_perturbation_type == "reverse_control":
            file_token_sequences = [seq + [marker_rev_token] for seq in file_token_sequences]
        elif args.perturbation_type == "reverse_partial" and args.test_perturbation_type == "reverse_full":
            file_token_sequences = [[marker_rev_token] + seq[::-1] for seq in file_token_sequences]
        elif args.perturbation_type == "reverse_partial" and args.test_perturbation_type == "reverse_partial":
            file_token_sequences = [seq[:len(seq)//2] + [marker_rev_token] + seq[len(seq)//2:][::-1] for seq in file_token_sequences]
    ...
    directory = f"perplexity_results_rev_test/{args.perturbation_type}_{args.train_set}"
